...
import { useAppState } from '../context'; // Import our app context hook
...
// Adjusted the validator map type parameters so functions can receive optional context
const VALIDATORS: Record<keyof BorrowFormData, (v: string, context?: string) => string | null> = {
  fullName: (v, emailAddress = '') => {
    ...
    // Security check: Verify that at least one part of their name is present in the graduate email prefix
    if (emailAddress.trim().toLowerCase().endsWith('@graduate.utm.my')) {
      const emailPrefix = emailAddress.trim().split('@')[0].toUpperCase();
      
      // We skip checking generic test accounts like student1, student2, etc.
  ...
  yearCourse: (v) => {
    ...
    if (!/^\d\s*[\/\-]\s*[A-Za-z]+$/.test(v) && !/^\d\s*[A-Za-z]+$/.test(v)) {
      // Accommodate flexible entries like "3 SKEE" or "3/SKELH" seen in UTM labs
      if (!/^\d\s*[\/\s]\s*[A-Za-z0-9]+$/.test(v)) return 'Format example: 3 SKEE or 3/SKELH';
    }
    return null;
  },
  duration: (v) => {
  ...
  phoneNumber: (v) => {
    ...
    if (digits.length < 9 || digits.length > 12) return 'Must be a valid active phone length sequence';
    return null;
  },
  emailAddress: (v) => {
    if (!v.trim()) return 'Required';
...
export default function BorrowForm({ equipmentCode, currentUserEmail, onSubmit, onBack }: BorrowFormProps) {
  const { getLastSubmittedForm } = useAppState(); // Pull previous profile metadata tracker

  // Initialize the state directly from memory if a previous form was submitted
  const [form, setForm] = useState<BorrowFormData>(() => {
    ...
    return {
      ...
      duration: '', // Reset fields to ensure fresh data for new applications
      returnTime: '16:00', // Default lab standard closing target
      dateBorrow: new Date().toISOString().split('T')[0],
      phoneNumber: savedForm?.phoneNumber || '',
      // SECURITY FIX: Lock the email address straight to the session credential
  ...
  const errors = useMemo(() => {
    ...
    for (const key of Object.keys(form) as (keyof BorrowFormData)[]) {
      // Pass the current email state as a context argument when checking the full name field
      const err = key === 'fullName'
  ...
  const fieldClass = (field: keyof BorrowFormData) =>
    `w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-utm-maroon/30 focus:border-utm-maroon transition-all ${
      touched[field] && errors[field] ? 'border-red-300 bg-red-50/50' : 'border-gray-200 bg-white'
    }`;

  return (
    <div className="space-y-6 animate-fade-in">
      {/* Back button navigation element banner layout */}
      <div className="flex items-center gap-4">
        <button
          onClick={onBack}
          type="button"
          className="w-9 h-9 rounded-lg border border-gray-200 bg-white flex items-center justify-center hover:bg-gray-50 transition-colors cursor-pointer"
        >
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">
            Target Hardware Code: <span className="font-mono font-semibold text-utm-maroon">{equipmentCode}</span>
          </p>
        </div>
      </div>

      {/* Form Context Control Layout Card */}
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden max-w-lg shadow-sm">
        <form onSubmit={handleSubmit} className="p-6 space-y-4" noValidate>
          ...
          <div>
            ...
            />
            {touched.fullName && errors.fullName && <p className="text-[10px] text-red-500 mt-1">{errors.fullName}</p>}
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">Year / Course</label>
            <input
              type="text"
              value={form.yearCourse}
              onChange={(e) => setForm({ ...form, yearCourse: e.target.value.toUpperCase() })}
              onBlur={() => handleBlur('yearCourse')}
              className={fieldClass('yearCourse')}
              placeholder="e.g. 3 SKEE or 3/SKELH"
            />
            {touched.yearCourse && errors.yearCourse && <p className="text-[10px] text-red-500 mt-1">{errors.yearCourse}</p>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                value={form.duration}
                onChange={(e) => setForm({ ...form, duration: e.target.value.toUpperCase() })}
                onBlur={() => handleBlur('duration')}
                className={fieldClass('duration')}
                placeholder="e.g. 1 DAY or 2 HOURS"
              />
              {touched.duration && errors.duration && <p className="text-[10px] text-red-500 mt-1">{errors.duration}</p>}
            </div>
            <div>
              ...
              />
              {touched.returnTime && errors.returnTime && <p className="text-[10px] text-red-500 mt-1">{errors.returnTime}</p>}
            </div>
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">Date Borrow</label>
            <input
              type="date"
            ...
            />
            {touched.dateBorrow && errors.dateBorrow && <p className="text-[10px] text-red-500 mt-1">{errors.dateBorrow}</p>}
          </div>
          ...
          <div>
            ...
            />
            {touched.phoneNumber && errors.phoneNumber && <p className="text-[10px] text-red-500 mt-1">{errors.phoneNumber}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              disabled={true} // SECURITY FIX: Prevent form manipulation by freezing input
            ...
            />
            <p className="text-[10px] text-emerald-600 mt-1 font-medium">✓ Locked to your active authenticated student session credentials.</p>
          </div>

          {/* New Photo Attachment Field Container */}
          <div className="pt-3 border-t border-gray-100">
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full text-xs text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-utm-maroon/10 file:text-utm-maroon hover:file:bg-utm-maroon/20 file:cursor-pointer cursor-pointer"
              />
              ...
              {photoAttachment && (
                <div className="relative inline-block border border-gray-200 rounded-xl bg-gray-50 p-1.5 pr-8 max-w-[200px]">
                  ...
                  <button
                    ...
                    className="absolute top-1 right-1 w-5 h-5 bg-gray-900/80 text-white rounded-full flex items-center justify-center hover:bg-gray-900 transition-colors cursor-pointer"
        ...
        {/* Footer System Compliance Reminder */}
        <div className="px-6 py-3 bg-utm-maroon/5 border-t border-utm-maroon/10">
          <p className="text-[11px] text-utm-maroon font-medium leading-relaxed">
            Please remember to update form before the duedate to avoid your name from being blacklisted by the system
          </p>
        </div>
      </div>
    </div>
