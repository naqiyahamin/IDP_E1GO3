...
// User Profile Template Definition
...
interface AppState {
  ...
  // FIXED ADDITIONS: State engines tracking passwords reactively
  dynamicPasswords: Record<string, string>;
  resetUserPassword: (email: string, newPassword: string) => boolean;
  verifyUserCredentials: (email: string, passcode: string) => { success: boolean; user?: UserProfile };
}

const AppContext = createContext<AppState | null>(null);

// Official Built-in Institutional Credentials Registry
const UTM_USER_REGISTRY: Record<string, UserProfile> = {
  'naqiyah@graduate.utm.my': {
    fullName: 'NAQIYAH BINTI AHMAD',
    role: 'STUDENT',
    defaultPasscode: 'stud_naqiyah2026'
  },
  'amirul@graduate.utm.my': {
    fullName: 'AMIRUL BIN MOHD',
    role: 'STUDENT',
    defaultPasscode: 'stud_amirul2026'
  },
  'divya@graduate.utm.my': {
    fullName: 'DIVYA A/P RAMAN',
    role: 'STUDENT',
    defaultPasscode: 'stud_divya2026'
  },
  'razali@utm.my': {
    fullName: 'RAZALI AHMAD',
    role: 'STAFF',
    defaultPasscode: 'staff_razali2026'
  },
  'norhayati@utm.my': {
    fullName: 'NORHAYATI IDRIS',
    role: 'STAFF',
    defaultPasscode: 'staff_norhayati2026'
  }
};
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  // Dynamic override state variable
  ...
  useEffect(() => { localStorage.setItem('utm_equipment_rows', JSON.stringify(equipmentRows)); }, [equipmentRows]);
  useEffect(() => { localStorage.setItem('utm_application_queue', JSON.stringify(applicationQueue)); }, [applicationQueue]);
  useEffect(() => { localStorage.setItem('utm_blacklisted_emails', JSON.stringify(blacklistedEmails)); }, [blacklistedEmails]);
  useEffect(() => { localStorage.setItem('utm_transaction_history', JSON.stringify(transactionHistory)); }, [transactionHistory]);
  ...
  // Multi-tier authentication layer checking defaults AND modifications
  const verifyUserCredentials = useCallback((email: string, passcode: string) => {
    const normalizedEmail = email.toLowerCase().trim();
    const userProfile = UTM_USER_REGISTRY[normalizedEmail];
    
    if (!userProfile) return { success: false };

    // Use the newly set password if it exists, otherwise fall back to the old default one
    const activePasscode = dynamicPasswords[normalizedEmail] || userProfile.defaultPasscode;
    
    if (activePasscode === passcode) {
  ...
  const updateEquipmentStatus = useCallback((code: string, newStatus: EquipmentStatus) => {
    setEquipmentRows((prev) => prev.map((row) => (row.code === code ? { ...row, status: newStatus } : row)));
  }, []);
  ...
  const submitApplication = useCallback((formData: BorrowFormData, equipmentCode: string, photoAttachment?: string) => {
    ...
    setEquipmentRows((prevRows) => prevRows.map((row) => row.code === equipmentCode ? { ...row, status: 'PENDING PICKUP' } : row));
  ...
  const approveApplication = useCallback((appId: string) => {
    setApplicationQueue((prevQueue) => {
      ...
      if (target) {
        setEquipmentRows((prevRows) => prevRows.map((row) => row.code === target.equipmentCode ? { ...row, status: 'BORROWED' } : row));
        setComponentInventory((prevInv) =>
          prevInv.map((item) => item.name === 'Digital Oscilloscope' ? { ...item, unitsOut: item.unitsOut + 1, unitsOnShelf: item.unitsOnShelf - 1 } : item)
        );
      }
      return prevQueue.map((a) =>
        a.id === appId
           ? { ...a, status: 'APPROVED', stage: 'ACTIVE_BORROW', isApproved: true, approvedAt: new Date().toISOString(), processedAt: new Date().toISOString() }
           : a
      );
    });
  }, []);
  ...
  const rejectApplication = useCallback((appId: string) => {
    setApplicationQueue((prevQueue) => {
      const target = prevQueue.find((a) => a.id === appId);
      if (target) {
        setEquipmentRows((prevRows) => prevRows.map((row) => row.code === target.equipmentCode ? { ...row, status: 'AVAILABLE' } : row));
      }
  ...
  const submitReturnRequest = useCallback((appId: string, returnDetails: ReturnDetailsData) => {
    setApplicationQueue((prevQueue) => {
      const target = prevQueue.find((a) => a.id === appId);
      if (target) {
        setEquipmentRows((prevRows) => prevRows.map((row) => row.code === target.equipmentCode ? { ...row, status: 'RETURN_PENDING' } : row));
      }
      return prevQueue.map((a) => a.id === appId ? { ...a, isReturned: true, returnDetails, returnSubmittedAt: new Date().toISOString() } : a);
    });
  }, []);
  ...
  const approveReturnRequest = useCallback((appId: string) => {
    setApplicationQueue((prevQueue) => {
      ...
      if (target) {
        ...
        setComponentInventory((prevInv) =>
          prevInv.map((item) => item.name === 'Digital Oscilloscope' ? { ...item, unitsOut: Math.max(0, item.unitsOut - 1), unitsOnShelf: item.unitsOnShelf + 1 } : item)
        );
      ...
      return prevQueue.map((a) =>
        a.id === appId ? { ...a, status: 'RETURNED', stage: 'HISTORICAL', isReturnVerified: true, processedAt: new Date().toISOString(), returnVerifiedAt: new Date().toISOString() } : a
      );
    });
  }, []);
  ...
  const incomingVerificationQueue = useMemo(() => applicationQueue.filter((app) => app.stage === 'PENDING'), [applicationQueue]);
  const processedApplicationsLog = useMemo(() => applicationQueue.filter((app) => app.stage === 'ACTIVE_BORROW'), [applicationQueue]);
  const historicalLedger = useMemo(() => applicationQueue.filter((app) => app.stage === 'HISTORICAL'), [applicationQueue]);
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        // Expose credentials functions to providers
        dynamicPasswords,
        resetUserPassword,
